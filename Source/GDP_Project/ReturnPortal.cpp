...
AReturnPortal::AReturnPortal()
{
	...
	LoungeReturnPointTriggerBox = CreateDefaultSubobject<UBoxComponent>(TEXT("LoungeReturnPointTriggerBox"));
	LoungeReturnPointTriggerBox->SetupAttachment(RootComponent);
	LoungeReturnPointTriggerBox->OnComponentBeginOverlap.AddDynamic(this, &AReturnPortal::OnBeginOverlap);
	LoungeReturnPointTriggerBox->SetBoxExtent(FVector(100.0f, 100.0f, 10.0f));
	...
	//For testing
	LoungeReturnPointWidget = CreateDefaultSubobject<UWidgetComponent>(TEXT("LoungeReturnPointWidget"));
	LoungeReturnPointWidget->SetupAttachment(LoungeReturnPointTriggerBox);
	static ConstructorHelpers::FClassFinder<UUserWidget> CameraWidget(TEXT("/Game/TestingPurpose/PuzzleComplete"));
	LoungeReturnPointWidget->SetWidgetClass(CameraWidget.Class);
	LoungeReturnPointWidget->SetRelativeLocation(FVector(0, 0, 150.0f));
	LoungeReturnPointWidget->SetTwoSided(true);
...
void AReturnPortal::Tick(float DeltaTime)
{
	...
	LoungeReturnPointWidget->SetRelativeRotation(PlayerRot);
